...
def accuracy(output, target, topk=(1,)):
    ...
    _, pred = tf.nn.top_k(output, maxk, sorted=True)
    pred = tf.transpose(pred)
    correct = tf.equal(pred, tf.reshape(target, (1, -1)))

    res = []
    for k in topk:
        correct_k = tf.reduce_sum(tf.cast(correct[:k], tf.float32))
        res.append(correct_k / tf.cast(batch_size, tf.float32))
    return res
...
def summary(model, loader, args):
    ...
    for batch_idx, (data, label, site, sex) in enumerate(loader):
        ...
        model_results_dict = model(data, sex, training=False)
        ...
        cls_logger.log(int(Y_hat.numpy()), int(label.numpy()))
        site_logger.log(int(site_hat.numpy()), int(site.numpy()))
        cls_probs = Y_prob.numpy()
        all_cls_probs[batch_idx] = cls_probs
        all_cls_labels[batch_idx] = float(label.numpy())

        all_sexes[batch_idx] = float(sex.numpy())
    ...
    if args.n_classes > 2:
        if args.n_classes > 5:
            ...
            topk = (1,3)
        topk_accs = accuracy(tf.convert_to_tensor(all_cls_probs), tf.convert_to_tensor(all_cls_labels), topk=topk)
