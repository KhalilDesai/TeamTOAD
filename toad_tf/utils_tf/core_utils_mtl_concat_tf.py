...
def train(datasets, cur, args):
    ...
    if args.early_stopping:
        model.load_weights(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    else:
        model.save_weights(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   
    ...
    for batch_idx, (data, label, site, sex) in enumerate(loader):
        ...
        cls_error = calculate_error(int(Y_hat.numpy()), int(label.numpy()))
        cls_train_error += cls_error
        site_error = calculate_error(int(site_hat.numpy()), int(site.numpy()))
        ...
        with tf.GradientTape() as tape:
            ...
            logits, Y_prob, Y_hat = results_dict['logits'], results_dict['Y_prob'], results_dict['Y_hat']
            site_logits, site_prob, site_hat = results_dict['site_logits'], results_dict['site_prob'], results_dict['site_hat']
            cls_loss = loss_fn(tf.reshape(label, (1,)), logits)
            site_loss = loss_fn(tf.reshape(site, (1,)), site_logits)
            loss = 0.75 * cls_loss + 0.25 * site_loss
            cls_loss_value = float(cls_loss.numpy())
            site_loss_value = float(site_loss.numpy())
        ...
        if (batch_idx + 1) % 5 == 0:
            print(f"batch {batch_idx}, cls loss: {cls_loss_value:.4f}, site loss: {site_loss_value:.4f}, "
                f"label: {float(label.numpy())}, site: {float(site.numpy())}, "
                f"sex: {float(sex.numpy())}, bag_size: {data.shape[0]}")
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None):
    ...
    for batch_idx, (data, label, site, sex) in enumerate(loader):
        ...
        cls_probs[batch_idx] = Y_prob.numpy()
        cls_labels[batch_idx] = float(label.numpy())

        site_probs[batch_idx] = site_prob.numpy()
        site_labels[batch_idx] = float(site.numpy())
...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, site, sex) in enumerate(loader):
        ...
        cls_logger.log(Y_hat, label)
        site_logger.log(site_hat, site)
        cls_probs = Y_prob.numpy()
        all_cls_probs[batch_idx] = cls_probs
        all_cls_labels[batch_idx] = float(label.numpy())

        site_probs = site_prob.numpy()
        all_site_probs[batch_idx] = site_probs
        all_site_labels[batch_idx] = float(site.numpy())
        
        patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'cls_prob': cls_probs, 'cls_label': float(label.numpy()), 
                                'site_prob': site_probs, 'site_label': float(site.numpy())}})
        cls_error = calculate_error(int(Y_hat.numpy()), int(label.numpy()))
